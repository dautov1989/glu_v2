...
<x-layouts.app title="Можно ли при сахарном диабете?">
    <script>
        ...
        window.initCanIEat = function () {
            return {
                search: '',
                activeCategory: 'sweets',
                items: window.canIEatData || [],
                categories: [
    ...
    <div class="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm overflow-hidden"
        ...
        <div class="border-b border-zinc-100 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-6 space-y-6">
            ...
            <div class="flex flex-col md:flex-row gap-4 justify-between items-start md:items-center">
                <div class="flex items-center gap-3">
                    <div
                        class="w-12 h-12 rounded-xl bg-emerald-100 dark:bg-emerald-900/30 flex items-center justify-center text-2xl shrink-0 text-emerald-600 dark:text-emerald-400">
                        🥗
                    </div>
                    <div>
                        ...
                        <p class="text-sm text-zinc-500 dark:text-zinc-400">
                            Интерактивный справочник продуктов
                        </p>
                    </div>
                </div>
                ...
                <div class="relative w-full md:w-96">
                    <input type="text" x-model="search" placeholder="Поиск (мед, яблоко)..."
                        class="w-full pl-10 pr-10 py-2.5 bg-zinc-50 dark:bg-zinc-800/50 border border-zinc-200 dark:border-zinc-700 rounded-lg focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all text-sm font-medium">
                ...
                <template x-for="cat in categories" :key="cat.id">
                    <button @click="activeCategory = cat.id"
                        :class="activeCategory === cat.id 
                            ...
                            : 'bg-white dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 border-zinc-200 dark:border-zinc-700 hover:border-emerald-400 dark:hover:border-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-900/10'"
        ...
        <!-- Legend -->
        <div
            class="px-6 py-3 bg-zinc-50 dark:bg-zinc-800/50 flex flex-wrap gap-4 text-[10px] md:text-xs border-b border-zinc-100 dark:border-zinc-800 text-zinc-500">
            <div class="flex items-center gap-1.5 font-bold text-emerald-600 dark:text-emerald-400">
                <div class="w-2 h-2 bg-emerald-500 rounded-full"></div> ПОЛЕЗНО
            </div>
            <div class="flex items-center gap-1.5 font-bold text-amber-600 dark:text-amber-400">
                <div class="w-2 h-2 bg-amber-500 rounded-full"></div> УМЕРЕННО
            </div>
            <div class="flex items-center gap-1.5 font-bold text-rose-600 dark:text-rose-400">
                <div class="w-2 h-2 bg-rose-500 rounded-full"></div> НЕЛЬЗЯ
            </div>
        </div>

        <!-- Results Grid -->
        <div class="p-6 md:p-8 min-h-[400px] bg-white dark:bg-zinc-900">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4" x-show="filteredItems.length > 0">
                <template x-for="item in filteredItems" :key="item.name">
                    <div :class="{
                        ...
                        class="p-5 rounded-2xl border transition-all duration-300 hover:scale-[1.02] flex flex-col group">
                        ...
                        <div class="flex items-center justify-between mb-3">
                            ...
                            <div :class="{
                                ...
                                }" class="w-2.5 h-2.5 rounded-full shadow-lg group-hover:animate-ping"></div>
                        </div>
