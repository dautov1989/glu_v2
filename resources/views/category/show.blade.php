@section('seo-meta')
    <x-seo-meta :title="$category->meta_title ?? $category->title" :description="$category->meta_description ?? $category->description" :keywords="'диабет, ' . $category->title . ', статьи о диабете'" type="website"
...
@section('content')
    @php
        ...
        $hasProducts = $categoryProducts->count() > 0;
    @endphp
    <div
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-6">
            ...
            @if($hasProducts || $category->children->count() > 0)
                <div class="mb-10">
                    <div class="relative overflow-hidden group/card">
                        @if($hasProducts)
                            {{-- Main Header (Root) --}}
                            <div class="relative z-10 flex items-start gap-4 md:gap-6 ml-1">
                                {{-- Content (Right) --}}
                        ...
                        {{-- Встраиваем товары прямо в карточку категории (горизонтальный ремешок) --}}
                        @if($hasProducts)
                            <x-product-showcase-v2 :category="$category" :products="$categoryProducts" />
                        @endif
                    </div>
                </div>
            @endif
                ...
                <!-- Posts Grid -->
                @if($posts->count() > 0)
                    <div class="mb-12" x-data="bentoLoader('{{ $posts->nextPageUrl() }}')">
                    <div class="mb-10">
                            <h2 id="materials-start" class="text-xl md:text-2xl font-bold text-zinc-800 dark:text-zinc-100 flex items-start mb-2 container-wide">
                                <span class="shrink-0 w-1.5 h-8 bg-gradient-to-b from-cyan-400 to-blue-600 rounded-full mr-3 shadow-lg shadow-cyan-500/30 mt-0.5 sm:mt-0"></span>
                                <div class="flex flex-wrap items-center gap-x-3 gap-y-2">
                                    <span class="leading-tight">
                                        @if($hasProducts)
                                            Материалы в этой категории
                                        @else
                                            Материалы в категории <span class="text-cyan-600 dark:text-cyan-400">«{{ $category->title }}»</span>
                                        @endif
                                    </span>
                                </div>
                            </h2>
                            @if($category->description)
                                <p class="text-zinc-600 dark:text-zinc-400 text-sm md:text-base leading-relaxed ml-4 border-l-2 border-zinc-200 dark:border-zinc-700 pl-4 mt-2">
